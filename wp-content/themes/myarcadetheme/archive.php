    ...
    <div id="ajaxcat">
      <main class="main-cn cols-n9">
        ...
        if (have_posts()) {
          ...
          $getCategory = $wpdb->get_results("SELECT * FROM wp_category_custom WHERE category_id = " . $category->term_id);
        ?>
          <div class="titl" style="font-weight: inherit;
    font-size: inherit;
    line-height: inherit;
    text-transform: inherit;">
            <div style="display: flex;">
              <div>
                <img src="<?php echo $getCategory[0]->icon ?>">
              </div>
              <div>
                <div><?php myarcadetheme_archive_title(); ?></div>
                <div><?php echo html_entity_decode($getCategory[0]->content) ?></div>
              </div>
            </div>
